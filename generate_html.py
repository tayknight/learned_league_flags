...
class FetchAndParseMembers:
    ...
    def requests_retry_with_session (
        self
        , retries=3
        , backoff_factor=0.3
    ...
    def get_session(self):
        ...
        ses1 = self.requests_retry_with_session()
        try:
    ...
    def fetch_flag(self, session, member, header):
        if member['player_flag']:
            ...
            if flag_extension == 'gif':
                ...
                try:
                    img_file = session.get(header + image_src)
                    img_file.raise_for_status()
                    with open(local_file_path, 'wb') as f:
                        f.write(img_file.content)
                except HTTPError as http_err:
                    return {'status': 'httpfail', 'member': member, 'error': http_err}
                except Exception as e:
                    return {'status': 'fail', 'tried': header + image_src, 'exception': traceback.format_exc()}
            return {'status': 'success', 'member': member }
        else:
            return {'status': 'missing', 'member': member}

    def fetch_player_data(self, session):
        # Get and parse scripts/playerdata.js
        try:
            playerdata = session.get(f'{self.LLHEADER}/scripts/playerdata.js')
    ...
    def fetch_flag_data(self, session):
        ...
        if self.FETCH_FLAGDATA == True:
            ...
            total_batches = math.floor((len(player_list) / self.FETCH_BATCH))
            ...
            while i < len(player_list):
                ...
                if len(small_list) % self.FETCH_BATCH == 0:
                    ...
                    for player in small_list:
                        small_results.append(pool.apply_async(self.fetch_player, args=(session, player, self.LLHEADER )))
                    ...
                    this_batch = (time() - ts)
                    print(f'Batch: {batch_count} of {total_batches}. Fetching {self.FETCH_BATCH} image URLs in: {this_batch}')
                    # Try to play nice...
    ...
    def fetch_flag_images(self):
        ...
        # How many members in each batch?
        total_batches = math.floor((len(member_data) / self.FETCH_BATCH))

        # Unloggedin session
        anon_session = self.requests_retry_with_session()
        ...
        while i < len(member_data):
            ...
            if len(small_list) % self.FETCH_BATCH == 0:
                ...
                for member in small_list:
                    small_results.append(pool.apply_async(self.fetch_flag, args=(anon_session, member, self.LLHEADER )))
                ...
                # How long did this batch take?
                this_batch = (time() - ts)
                print(f'Batch: {batch_count} of {total_batches}. Fetching {self.FETCH_BATCH} images in: {this_batch}')
                # Try to play nice...
                sleep(self.FETCH_SLEEP)
        ...
        for result in results:
            ...
            if result['status'] == 'fail':
                print('Image fetch failed for {}. {}'.format(result['member']['player_name'], result['exception']))
                fail += 1
            elif result['status'] == 'httpfail':
                print('Image fetch failed for {}. {}'.format(result['member']['player_name'], result['error']))
                httpfail += 1
            elif result['status'] == 'missing':
                print('Image fetch failed for {}.'.format(result['member']['player_name']))
                missing += 1
            else:
                success += 1
    ...
    def generate_players_js(self):
        ...
        for member in member_data:
            ...
            memberProfile = self.LLHEADER + member['player_link']
            if member['player_flag'] == None:
                flagUrl = ''
            else:
                flagUrl = self.LLHEADER + member['player_flag']
...
def main():
    ...
    if ll.FETCH_PLAYERDATA:
        ll.fetch_player_data(session)

    if ll.FETCH_FLAGDATA:
        ll.fetch_flag_data(session)
